...
import { Download, Copy, Share2, Sparkles, FileText, ChevronRight, CheckCircle2 } from "lucide-react";
      ...
      <Tabs defaultValue="dashboard">
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" className="text-xs h-8" data-testid="export-pdf"><Download size={12} className="mr-1.5" />Export PDF</Button>
                      <Button variant="outline" size="sm" className="text-xs h-8" data-testid="export-csv">Export CSV</Button>
                      <Button variant="outline" size="sm" className="text-xs h-8" data-testid="copy-narrative"><Copy size={12} className="mr-1.5" />Copy Narrative</Button>
                      <Button size="sm" className="text-xs h-8" data-testid="share-report"><Share2 size={12} className="mr-1.5" />Share</Button>
                    </div>
                  </div>
                      ...
                      {[
                        {
                          ...
                          content: "Atlanta Workforce Tech Alliance delivered technology career programming to 127 learners during Q2 2026 across three cohort programs. This report documents measurable outcomes in learner engagement, skill development, workforce readiness, and placement."
                        },
        ...
        <TabsContent value="metrics">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {[
              {
                group: "Engagement", color: "blue",
                metrics: [
                  { name: "Active Learners", value: 83, max: 127, unit: "" },
                  { name: "Login Consistency", value: 74, max: 100, unit: "%" },
                  { name: "Event Participation", value: 74, max: 100, unit: "%" },
                  { name: "Coach Check-ins", value: 82, max: 100, unit: "%" },
                ]
              },
              {
                group: "Progress", color: "emerald",
                metrics: [
                  { name: "Roadmap Completion", value: 63, max: 100, unit: "%" },
                  { name: "Milestone Completion", value: 71, max: 100, unit: "%" },
                  { name: "Project Completion", value: 68, max: 100, unit: "%" },
                  { name: "Profile Completion", value: 76, max: 100, unit: "%" },
                ]
              },
              {
                group: "Readiness", color: "indigo",
                metrics: [
                  { name: "Resume Readiness", value: 72, max: 100, unit: "" },
                  { name: "Interview Readiness", value: 65, max: 100, unit: "" },
                  { name: "Portfolio Readiness", value: 70, max: 100, unit: "" },
                  { name: "Technical Confidence", value: 68, max: 100, unit: "" },
                  { name: "Communication Readiness", value: 74, max: 100, unit: "" },
                ]
              },
              {
                group: "Workforce Outcomes", color: "teal",
                metrics: [
                  { name: "Applications Submitted", value: 34, max: 83, unit: "" },
                  { name: "Interviews Completed", value: 28, max: 34, unit: "" },
                  { name: "Offers Received", value: 12, max: 28, unit: "" },
                  { name: "Placements Achieved", value: 9, max: 12, unit: "" },
                  { name: "Avg Weeks to Placement", value: 6, max: 20, unit: " wks" },
                ]
              },
            ].map(group => (
                  ...
                  {group.metrics.map(m => (
                    <div key={m.name}>
                      <div className="flex justify-between text-sm mb-1.5">
                        ...
                        <span className="font-semibold text-foreground text-xs">{m.value}{m.unit}</span>
                      </div>
              ...
              <Card key={report.type} data-testid={`export-card-${report.type.toLowerCase().replace(/\s+/g, "-")}`} className="border-card-border shadow-sm">
                <CardContent className="p-5">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <Button size="sm" className="text-xs h-8 shrink-0 ml-3" data-testid={`export-${report.type.toLowerCase().replace(/\s+/g, "-")}`}>
